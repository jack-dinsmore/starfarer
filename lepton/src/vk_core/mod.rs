...
/// A struct for containing Vulkan information
...
impl VulkanData {
    pub fn new(event_loop: &winit::event_loop::EventLoop<()>) -> VulkanData {
        ...
        let msaa_samples = VulkanData::get_max_usable_sample_count(&instance, physical_device);
        ...
        let render_pass = VulkanData::create_render_pass(&instance, &device, physical_device, swapchain_stuff.swapchain_format, msaa_samples);
        let ubo_layout = share::v2::create_descriptor_set_layout(&device);
        let (graphics_pipeline, pipeline_layout) = VulkanData::create_graphics_pipeline(&device, render_pass,
            swapchain_stuff.swapchain_extent, ubo_layout, msaa_samples);
        let command_pool = share::v1::create_command_pool(&device, &queue_family);
        let (color_image, color_image_view, color_image_memory) = VulkanData::create_color_resources(
                &device, swapchain_stuff.swapchain_format, swapchain_stuff.swapchain_extent, &physical_device_memory_properties, msaa_samples);
        let (depth_image, depth_image_view, depth_image_memory) = share::v1::create_depth_resources(&instance, &device, physical_device,
            command_pool, graphics_queue, swapchain_stuff.swapchain_extent, &physical_device_memory_properties, msaa_samples);
        let swapchain_framebuffers = VulkanData::create_framebuffers(
        ...
        let (texture_image, texture_image_memory, mip_levels) = VulkanData::create_texture_image(&device, command_pool,
            graphics_queue, &physical_device_memory_properties, &Path::new(TEXTURE_PATH));
        let texture_image_view = share::v1::create_texture_image_view(&device, texture_image, mip_levels);
        let texture_sampler = VulkanData::create_texture_sampler(&device, mip_levels);
        let (vertex_buffer, vertex_buffer_memory) = share::v1::create_vertex_buffer( &device,
            &physical_device_memory_properties, command_pool, graphics_queue, &vertices);
        let (index_buffer, index_buffer_memory) = share::v1::create_index_buffer(
            &device, &physical_device_memory_properties, command_pool, graphics_queue, &indices);
        ...
        let descriptor_sets = share::v2::create_descriptor_sets(
            &device, descriptor_pool, ubo_layout, &uniform_buffers, texture_image_view, texture_sampler, swapchain_stuff.swapchain_images.len(),
        );
        let command_buffers = VulkanData::create_command_buffers(&device, command_pool, graphics_pipeline, &swapchain_framebuffers,
            render_pass, swapchain_stuff.swapchain_extent, vertex_buffer, index_buffer, pipeline_layout, &descriptor_sets, indices.len() as u32);
    ...
    /// What is the maximum sample count supported by the device? For multisampling.
    ...
    // Load a texture
    fn create_texture_image(device: &ash::Device, command_pool: vk::CommandPool, submit_queue: vk::Queue,
        device_memory_properties: &vk::PhysicalDeviceMemoryProperties, image_path: &Path) -> (vk::Image, vk::DeviceMemory, u32) {
...
impl VulkanData {
    ...
    fn create_graphics_pipeline(device: &ash::Device, render_pass: vk::RenderPass, swapchain_extent: vk::Extent2D,
        ubo_set_layout: vk::DescriptorSetLayout, msaa_samples: vk::SampleCountFlags) -> (vk::Pipeline, vk::PipelineLayout) {
        let vert_shader_module = share::create_shader_module(
            device,
            include_glsl!("src/shaders/26-shader-depth.vert", kind: vert).to_vec(),
        );
        let frag_shader_module = share::create_shader_module(
            device,
            include_glsl!("src/shaders/26-shader-depth.frag", kind: frag).to_vec(),
        );
    ...
    fn create_command_buffers(device: &ash::Device, command_pool: vk::CommandPool, graphics_pipeline: vk::Pipeline, framebuffers: &Vec<vk::Framebuffer>,
        render_pass: vk::RenderPass, surface_extent: vk::Extent2D, vertex_buffer: vk::Buffer, index_buffer: vk::Buffer, pipeline_layout: vk::PipelineLayout,
        descriptor_sets: &Vec<vk::DescriptorSet>, index_count: u32) -> Vec<vk::CommandBuffer> {
...
impl VulkanApp for VulkanData {
    ...
    fn recreate_swapchain(&mut self) {
        ...
        self.render_pass = VulkanData::create_render_pass(
            &self.instance,
            &self.device,
            self.physical_device,
            self.swapchain_format,
        ...
        let (graphics_pipeline, pipeline_layout) = VulkanData::create_graphics_pipeline(
            &self.device,
            self.render_pass,
        ...
        let color_resources = VulkanData::create_color_resources(
            &self.device,
            self.swapchain_format,
        ...
        self.swapchain_framebuffers = VulkanData::create_framebuffers(
            &self.device,
            self.render_pass,
        ...
        self.command_buffers = VulkanData::create_command_buffers(
            &self.device,
            self.command_pool,
